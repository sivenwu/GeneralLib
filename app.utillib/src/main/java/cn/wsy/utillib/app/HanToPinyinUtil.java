...
public class HanToPinyinUtil {
    ...
    public static String getPinYin(String input) {
        ...
        ArrayList<Token> tokens = HanToPinyinUtil.getInstance().get(input);
    ...
    private static final String TAG = "HanToPinyinUtil";
    ...
    private static HanToPinyinUtil sInstance;
    ...
    protected HanToPinyinUtil(boolean hasChinaCollator) {
        mHasChinaCollator = hasChinaCollator;
    }


    public static HanToPinyinUtil getInstance() {
        synchronized (HanToPinyinUtil.class) {
            if (sInstance != null) {
                return sInstance;
            }
            ...
            for (int i = 0; i < locale.length; i++) {
                if (locale[i].equals(Locale.CHINA)) {
                    ...
                    sInstance = new HanToPinyinUtil(true);
                    return sInstance;
                }
            }
            Log.w(TAG,
                    "There is no Chinese collator, HanToPinyinUtil is disabled");
            sInstance = new HanToPinyinUtil(false);
            return sInstance;
        }
    }
